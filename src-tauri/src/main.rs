...
use tauri::{
  ...
  tray::{TrayIcon, TrayIconEvent, TrayMenu, TrayMenuItem, Tray},
};
...
#[tauri::command]
fn copy_to_clipboard(text: String, _label: String) -> bool {
  #[cfg(target_os = "macos")]
  {
...
fn close_welcome(app: AppHandle, dont_show_again: bool) -> bool {
  if dont_show_again {
    if let Some(path) = app.path().config_dir() {
      let welcomed_path = path.join(".network-menubar-welcomed");
      let _ = std::fs::write(welcomed_path, SystemTime::now()
